...
class PaymentController extends Controller
{
    ...
    {
        return Payment::leftJoin('patients as p','p.id','=','payments.patient_id')->orderBy('payments.id','desc')->paginate(request()->toGet);
    }
    ...
    {
        $entrées = Payment::leftJoin("factures as f",'f.id','=',"payments.facture_id")
        ->leftJoin("consultations as c",'c.id','f.consultation_id')
        ->leftJoin("users as u",'u.id','=','c.doctor_id')
        ->select("payments.amount")
        ->get();
		
		$unadmitted = Payment::where("patient_id","=",0)->get();
        ...
        return ['charges'=>$charges, 'entries'=>$entrées ,'chiffre_affaires'=>$factures ,'mca'=>$mca ,'dca'=>$dca ,'wL'=>$wL ,'patients'=>$tP ,'unadmitted'=>$unadmitted];
    }
